...
/*
  ...
  Abaixo você verá uma série de testes e passos que irão guiar você e, que devem NECESSARIAMENTE ser realizados em ordem 
  ...
  A função createMenu() então, retornará um novo objeto. Este novo objeto contém algumas chaves e ao acessar cada uma 
  delas temos os seguintes retornos:
  ...
  - Uma chave `order` armazenando uma função. Essa função recebe uma string como parâmetro e essa string deve ser 
  adicionada à lista armazenada em `consumption`.

  - Uma chave `pay` que, quando chamada, invoca uma função. Essa função faz a soma do preço de todos os pedidos, 
  retornando essa soma de preços com acréscimo de 10%.
...
const checkIfMatch = (obj, x, currBill) => {
  const consumedItems = restaurant.consumption;
  for (let i = 0; i < consumedItems.length; i += 1) {
    if (x === consumedItems[i]) {
      currBill += obj[x];
    }
  }
  return currBill; 
...
const addtoBill = (obj, currBill) => {
  for (const x in obj) {
    if ({}.hasOwnProperty.call(obj, x)) {
      currBill = checkIfMatch(obj, x, currBill);
    }
  }
  return currBill;
};
...
const closeBill = () => {
  const itemsList = restaurant.fetchMenu();
  let totalBill = 0;
  for (const x in itemsList) {
    if ({}.hasOwnProperty.call(itemsList, x)) {
      const tempList = itemsList[x];
      totalBill = addtoBill(tempList, totalBill);
    }
  }
